...
static void count_command(InspectStats *s, const char *cmd) {
    ...
    if (strcmp(cmd, "clear") == 0 || strcmp(cmd, "grid") == 0 || strcmp(cmd, "line") == 0 || strcmp(cmd, "rect") == 0 || strcmp(cmd, "fillrect") == 0 || strcmp(cmd, "circle") == 0 || strcmp(cmd, "fillcircle") == 0 || strcmp(cmd, "tri") == 0 || strcmp(cmd, "wiretri") == 0 || strcmp(cmd, "ztri") == 0 || strcmp(cmd, "face") == 0 || strcmp(cmd, "wireface") == 0 || strcmp(cmd, "quad") == 0 || strcmp(cmd, "wirequad") == 0) { s->primitive++; return; }
    s->unknown++;
}
...
int dgl_inspect_file(const char *path) {
    ...
    memset(&s, 0, sizeof(s));
    if (f == NULL) { fprintf(stderr, "deadgl-inspect: cannot open %s\n", path); return 1; }
    while (fgets(line, sizeof(line), f) != NULL) {
        char *p;
        char *cmd;
        s.lines++;
        if (blank_or_comment(line)) { continue; }
        if (strchr(line, '\n') == NULL && !feof(f)) { fprintf(stderr, "deadgl-inspect: %s:%lu: line too long\n", path, s.lines); fclose(f); return 1; }
        p = skip_ws(line);
        cmd = strtok(p, " \t\r\n");
        if (cmd != NULL) { count_command(&s, cmd); }
    }
    fclose(f);
